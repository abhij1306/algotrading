...
def compute_features(symbol: str, hist: pd.DataFrame) -> dict:
    ...
    features["z_atr_pct"] = float(
        zscore(features["atr_pct"], hist["Close"].pct_change().abs().dropna())
    )
    ...
    features["trend_7d"] = (
        float(hist["Close"].pct_change(5).iloc[-1] * 100) if len(hist) >= 6 else 0.0
    )
    # 30D Trend (~21 trading days)
    features["trend_30d"] = (
        float(hist["Close"].pct_change(21).iloc[-1] * 100) if len(hist) >= 22 else 0.0
    )
    ...
    features["macd_signal"] = (
        float(recent["macd_signal"]) if not pd.isna(recent["macd_signal"]) else 0.0
    )
