...
class DataProvider:
    ...
    def get_intraday_data(self, symbols: list[str], target_date: date, timeframe: int = 5) -> dict[str, list]:
        ...
        candles = self.db.query(IntradayCandle).filter(
    ...
    def get_daily_data(self, symbols: list[str], start_date: date, end_date: date) -> dict[str, list]:
    ...
    def get_daily_data_chunked(self, symbols: list[str], start_date: date, end_date: date, chunk_days: int = 30):
    ...
    def get_latest_price(self, symbol: str) -> float | None:
        ...
        # Try intraday first
        candle = self.db.query(IntradayCandle).filter(
            IntradayCandle.symbol == symbol
        ).order_by(IntradayCandle.timestamp.desc()).first()

        if candle:
            return candle.close

        # Fallback to daily
        daily = self.db.query(HistoricalPrice).filter(
    ...
    def get_symbols_in_universe(self, universe_code: str, target_date: date) -> list[str]:
        ...
        universe = self.db.query(IndexUniverseDefinition).filter(
        ...
        constituents = self.db.query(IndexConstituentHistory).filter(
