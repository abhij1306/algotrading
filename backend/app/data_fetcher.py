"""
Data fetching module for yfinance and Fyers API
"""
...
def fetch_fyers_historical(symbol: str, days: int = 365) -> Optional[pd.DataFrame]:
    """
    Fetch historical data from Fyers API (much faster than yfinance)
...
def fetch_yfinance_data(symbol: str, period: str = '1y') -> Optional[pd.DataFrame]:
    """
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            hist = yf.download(ticker, period=period, interval='1d', progress=False, show_errors=False)
...
def fetch_historical_data(symbol: str, days: int = 365) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        elif days <= 3650: period = "10y"
        else: period = "max"

        hist = fetch_yfinance_data(symbol, period=period)
        if hist is not None and not hist.empty:
            # Save to database
...
def get_enhanced_quote(symbol: str, hist_data: pd.DataFrame) -> Dict:
    """
    Get enhanced quote combining yfinance historical + Fyers real-time
    ...
    if config.HAS_FYERS:
        try:
            ...
            pass  # Fallback to yfinance data
