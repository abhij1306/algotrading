...
from ...database import PaperFund, PaperPosition, PaperTrade, SessionLocal
from ...database import PaperOrder as OrderModel
...
class PaperBroker(IBroker):
    ...
    def _init_funds(self):
        ...
        try:
            ...
            if not fund:
                fund = PaperFund(
                    user_id=self.user_id, available_balance=1000000.0, total_balance=1000000.0
                )
                db.add(fund)
                db.commit()
        finally:
    ...
    def place_order(self, order: dict[str, Any]) -> OrderResponse:
        ...
        try:
            ...
            return OrderResponse(order_id="", status="REJECTED", message=str(e), details=None)
        finally:
            db.close()
        ...
        try:
            ...
            return [
                Position(
                    ...
                    pnl=((p.ltp or p.average_price) - p.average_price) * p.quantity
                    if p.side == "LONG"
                    else (p.average_price - (p.ltp or p.average_price)) * p.quantity,
