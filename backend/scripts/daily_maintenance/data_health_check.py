...
def check_database_integrity():
    ...
    db = SessionLocal()
    try:
        from app.database import Company, FinancialStatement, HistoricalPrice
...
def check_fyers_token():
    ...
    try:
        ...
        print("✅ Fyers Token: Valid")
        print(f"   User: {profile.get('name', 'Unknown')}")
        return True

    except Exception as e:
        if "token" in str(e).lower() or "401" in str(e):
            print("❌ Fyers Token: EXPIRED or INVALID")
            print("   Action Required: Re-authenticate at /api/auth/fyers")
        else:
...
def check_data_freshness():
    ...
    try:
        ...
        if days_old > 2:
            ...
            print("   Action Required: Run update_bhavcopy.py")
            return False
