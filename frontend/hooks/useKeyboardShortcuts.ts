  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "," && modKey) {
        ...
        // Settings shortcut - implementation pending
      }

      // ?: Help modal
      if (e.key === "?" && !modKey) {
        e.preventDefault();
        // TODO(#2): Show keyboard shortcuts help modal
        // Help shortcut - implementation pending
      }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowUp") {
        ...
        document.dispatchEvent(new CustomEvent("table-nav", { detail: { index: selectedIndex } }));
      }
